...
const getCryptoList = async (config?: RequestInit | undefined): TResponseAPI<TCryptoList> => {
	try {
		const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/coins/markets?vs_currency=usd&per_page=250&x_cg_demo_api_key=${process.env.NEXT_PUBLIC_API_KEY}`, config);
...
const getCryptoDetailsById = async (id: string, config?: RequestInit | undefined): TResponseAPI<ICryptoDetails | null> => {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_BASE_URL}/coins/${id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&x_cg_demo_api_key=${process.env.NEXT_PUBLIC_API_KEY}`,
			config,
		);
...
const getChartData = async (id: string, lastDays: number, config?: RequestInit | undefined): TResponseAPI<ICryptoChartData | null> => {
	try {
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_BASE_URL}/coins/${id}/market_chart?vs_currency=usd&days=${lastDays}&x_cg_demo_api_key=${process.env.NEXT_PUBLIC_API_KEY}`,
			config,
		);
		if (!response.ok) return { status: response.status, success: false, data: null };
...
const searchCrypto = async (term: string, config?: RequestInit | undefined): TResponseAPI<ISearchResult | null> => {
	try {
		const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/search?query=${term}&x_cg_demo_api_key=${process.env.NEXT_PUBLIC_API_KEY}`, config);
		if (!response.ok) return { status: response.status, success: false, data: null };
